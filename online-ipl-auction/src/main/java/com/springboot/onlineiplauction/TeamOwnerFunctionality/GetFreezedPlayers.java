...
import com.springboot.onlineiplauction.AdminFunctionality.AdminResponse;


@RestController
@ComponentScan(basePackages = {"onlineiplauction"})
public class  GetFreezedPlayers {

	@RequestMapping(value = "/getFreezedPlayers", method = RequestMethod.GET)
	public @ResponseBody
	ResponseEntity<CommonResponse> getFreezedPlayers() {
		...
		if (connection.SQLDatabaseConnection() != null) {
			...
			String query = "select * from playerdetails where isPlayer = 1 and isFreezed = 1 and Status = 1";
